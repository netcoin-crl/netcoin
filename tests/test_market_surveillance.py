...
def test_market_surveillance_flags_concentration_and_disputes(tmp_path: Path):
    ...
    store.place_market_order(mid, {"trader": "demo:bob", "demo_wallet": True, "side": "sell", "outcome_id": "out1", "price_bps": 5000, "quantity": 10})
    traded = store.place_market_order(mid, {"trader": "demo:alice", "demo_wallet": True, "side": "buy", "outcome_id": "out1", "price_bps": 5000, "quantity": 10})
    alerts = traded["surveillance"]["alerts"]
    assert any(a["code"] == "volume_concentration" for a in alerts)
    disputed = store.dispute_market_resolution(mid, {"actor": "alice", "reason": "source is ambiguous", "evidence_url": "https://example.com/evidence"})
