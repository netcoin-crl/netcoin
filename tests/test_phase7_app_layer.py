...
def test_phase7_templates_recurring_escrow_polls_and_markets(tmp_path: Path):
    ...
    timelock = store.create_contract({"type": "timelock", "public_key": buyer.public_key_hex, "unlock_height": 50, "amount": "1"})
    assert timelock["derived"]["address"]
    multisig = store.create_contract({"type": "multisig", "required_signatures": 2, "public_keys": [buyer.public_key_hex, seller.public_key_hex, mediator.public_key_hex]})
    ...
    recurring = store.create_recurring_agreement({"payer_address": buyer.address, "recipient_address": seller.address, "amount": "0.5", "interval": "weekly", "memo": "hosting"})
    ...
    first = store.escrow_action(escrow["escrow_id"], {"action": "release", "signer": "buyer", "to_address": seller.address})
    assert first["status"] == "pending_release"
    second = store.escrow_action(escrow["escrow_id"], {"action": "release", "signer": "seller", "to_address": seller.address})
    ...
    result = store.cast_poll_vote(poll["poll_id"], {"voter_address": buyer.address, "option_id": option_id, "signature": sig})
    ...
    market = store.create_prediction_market({"question": "Will NetCoin mine 10 blocks today?", "outcomes": ["YES", "NO"], "oracle": "manual"})
    yes = market["outcomes"][0]["outcome_id"]
    store.place_market_order(market["market_id"], {"trader_address": seller.address, "outcome_id": yes, "side": "sell", "quantity": 3, "price_bps": 4000})
    matched = store.place_market_order(market["market_id"], {"trader_address": buyer.address, "outcome_id": yes, "side": "buy", "quantity": 2, "price_bps": 5000})
