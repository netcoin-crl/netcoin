...
def flaky_peer():
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            length = int(self.headers.get("Content-Length", "0"))
            self.rfile.read(length)
            if self.path == "/block" and counts["failed"] == 0:
                counts["failed"] += 1
...
def test_broadcast_block_is_deduplicated(tmp_path: Path):
    ...
    node = NetCoinNode(chain, peers=[url], persist=False, request_retries=0)
    try:
...
def test_broadcast_transaction_is_deduplicated(tmp_path: Path):
    miner = Wallet.create()
    ...
    try:
        assert node.broadcast_transaction(tx) == 1
        assert counts["tx"] == 1
        assert node.broadcast_transaction(tx) == 0
        assert counts["tx"] == 1
    finally:
        server.shutdown()
        server.server_close()
...
def test_failed_relay_stays_queued_and_can_be_drained(tmp_path: Path):
    chain = Blockchain(tmp_path / "chain")
    miner = Wallet.create()
    block = solve_template(chain.get_block_template(miner_address=miner.address), miner.address)

    server, thread, url, counts = flaky_peer()
    node = NetCoinNode(chain, peers=[url], persist=False, request_retries=0)
    node.relay_backoff_seconds = 0
    try:
        assert node.broadcast_block(block) == 0
        assert len(node._relay_queue) == 1
        assert counts["failed"] == 1
        assert node.drain_relay_queue() == 1
        assert len(node._relay_queue) == 0
        assert counts["block"] == 1
    finally:
        server.shutdown()
        server.server_close()
