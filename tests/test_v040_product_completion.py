...
def test_completion_assets_include_functional_markers() -> None:
    ...
    for marker in ["NetCoinProductCompletion", "buildCommandPalette", "buildNotificationCenter", "mountSurfaceCompletion", "recordLocalNote"]:
        assert marker in js
    for marker in ["nc-command-palette", "nc-notification-center", "nc-upgrade-panel", "nc-timeline", "nc-status-badge", "nc-mobile-table"]:
...
def test_product_completion_tool_runs() -> None:
    result = subprocess.run([sys.executable, "tools/check_product_completion.py"], cwd=ROOT, text=True, capture_output=True, timeout=30)
