...
def _funded_chain(tmp_path: Path, blocks: int = 3) -> Blockchain:
    chain = Blockchain(tmp_path / "data", backend="json")
    miner = Wallet.create()
...
def test_recovers_from_backup_without_losing_blocks(tmp_path: Path):
    ...
    reopened = Blockchain(tmp_path / "data", backend="json")
    ...
    assert reopened.chain_path.read_text() == (chain.chain_path.parent / (chain.chain_path.name + ".bak")).read_text()
...
def test_unrecoverable_chain_raises_clearly(tmp_path: Path):
    ...
    with pytest.raises(ChainError, match="corrupt or unreadable"):
        Blockchain(tmp_path / "data", backend="json")
