use netcoin_node::run_p2p_parity_vectors;
use serde_json::Value;
use std::{env, fs, process};

fn main() {
    let Some(path) = env::args().nth(1) else {
        eprintln!("usage: netcoin-p2p-parity <parity-vectors.json>");
        process::exit(2);
    };
    ...
    let report = run_p2p_parity_vectors(&vectors);
    println!("{}", serde_json::to_string_pretty(&report).expect("serialize p2p parity report"));
