#!/usr/bin/env python3
"""Run/compare the Rust markets-core parity executable against Python reference vectors."""
...
from netcoin.parity_suite import run_market_vectors, vector_fingerprint  # noqa: E402

VECTOR_PATH = ROOT / "architecture" / "parity-vectors.json"
RUST_FIXTURE_PATH = ROOT / "core-rs" / "fixtures" / "parity-vectors.json"
RUST_CRATE_PATH = ROOT / "core-rs/crates/markets-core"
RUST_BIN_PATH = RUST_CRATE_PATH / "src" / "bin" / "netcoin-markets-parity.rs"
...
def _source_checks(vectors: dict[str, Any]) -> list[str]:
    ...
    for symbol in [
        ...
        if symbol not in lib_text:
            issues.append(f"Rust markets-core lib missing {symbol}")
    for symbol in ["run_markets_parity_vectors", "serde_json::to_string_pretty", "process::exit"]:
        if symbol not in bin_text:
            issues.append(f"Rust markets-core binary missing {symbol}")
    if "crates/markets-core" not in workspace_text:
        issues.append("core-rs workspace does not include crates/markets-core")
    if vectors.get("markets", {}).get("vector_set") != "markets-core-executable-vectors-v4":
        issues.append("markets vector_set is not markets-core-executable-vectors-v4")
    return issues


def _python_reference(vectors: dict[str, Any]) -> dict[str, dict[str, Any]]:
    results = [item.to_dict() for item in run_market_vectors(vectors)]
...
def _run_cargo(timeout: int) -> tuple[int, str, str]:
    cmd = [
        ...
        "-q",
        "-p",
        "netcoin-markets-core",
        "--bin",
        "netcoin-markets-parity",
        "--",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run NetCoin Rust markets-core parity executable and compare with Python"
    )
    parser.add_argument("--out", default="reports/rust_markets_parity_report.json")
    ...
    report: dict[str, Any] = {
        ...
        "markets_cases": len(vectors.get("markets", {}).get("cases", [])),
    ...
    summary_keys = ["ok", "mode", "cargo_available", "schema_version", "markets_cases", "vector_fingerprint"]
