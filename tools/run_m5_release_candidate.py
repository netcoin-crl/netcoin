#!/usr/bin/env python3
"""Run the M5 mainnet-launch release-candidate gate."""
...
def main() -> int:
    ...
    parser.add_argument("--out", default="reports/m5_release_candidate_report.json")
    ...
    if args.dry_run:
        ...
        result = {
            ...
            "run_count": len(runs),
            "status_counts": {status: len([item for item in runs if item["status"] == status]) for status in statuses},
            "runs": runs,
            "strict_evidence_required_for_operational_m5": args.profile == "strict",
            "does_not_deploy_or_mine_genesis": True,
        }
