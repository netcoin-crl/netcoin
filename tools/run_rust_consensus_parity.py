#!/usr/bin/env python3
"""Run/compare the Rust consensus parity executable against Python reference vectors.
...
and compares each Rust consensus result with the Python reference parity suite.
...
from netcoin.parity_suite import run_consensus_vectors, vector_fingerprint  # noqa: E402

VECTOR_PATH = ROOT / "architecture" / "parity-vectors.json"
RUST_FIXTURE_PATH = ROOT / "core-rs" / "fixtures" / "parity-vectors.json"
RUST_BIN_PATH = ROOT / "core-rs" / "crates" / "consensus" / "src" / "bin" / "netcoin-consensus-parity.rs"
RUST_LIB_PATH = ROOT / "core-rs" / "crates" / "consensus" / "src" / "lib.rs"
...
def _source_checks(vectors: dict[str, Any]) -> list[str]:
    issues: list[str] = []
    for path in [VECTOR_PATH, RUST_FIXTURE_PATH, RUST_BIN_PATH, RUST_LIB_PATH]:
        if not path.exists():
    ...
    for symbol in ["run_consensus_case", "run_consensus_parity_vectors", "consensus_actual_for_case"]:
        if symbol not in lib_text:
            issues.append(f"Rust consensus lib missing {symbol}")
    for symbol in ["run_consensus_parity_vectors", "serde_json::to_string_pretty", "process::exit"]:
        if symbol not in bin_text:
            issues.append(f"Rust parity binary missing {symbol}")
    kinds = {str(case.get("kind")) for case in vectors.get("consensus", {}).get("cases", [])}
    for kind in sorted(kinds):
        if f'"{kind}"' not in lib_text:
            issues.append(f"Rust consensus executable path does not mention vector kind {kind}")
    return issues


def _python_reference(vectors: dict[str, Any]) -> dict[str, dict[str, Any]]:
    results = [item.to_dict() for item in run_consensus_vectors(vectors)]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run NetCoin Rust consensus parity executable and compare with Python")
    parser.add_argument("--out", default="reports/rust_consensus_parity_report.json")
    ...
    report: dict[str, Any] = {
        ...
        "consensus_cases": len(vectors.get("consensus", {}).get("cases", [])),
    ...
    summary_keys = ["ok", "mode", "cargo_available", "schema_version", "consensus_cases", "vector_fingerprint"]
