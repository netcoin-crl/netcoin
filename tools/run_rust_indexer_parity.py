#!/usr/bin/env python3
"""Run/compare the Rust indexer-core parity executable against Python reference vectors."""
...
from netcoin.parity_suite import run_indexer_vectors, vector_fingerprint  # noqa: E402

VECTOR_PATH = ROOT / "architecture" / "parity-vectors.json"
RUST_FIXTURE_PATH = ROOT / "core-rs" / "fixtures" / "parity-vectors.json"
RUST_CRATE_PATH = ROOT / "core-rs/crates/indexer-core"
RUST_BIN_PATH = RUST_CRATE_PATH / "src" / "bin" / "netcoin-indexer-parity.rs"
...
def _source_checks(vectors: dict[str, Any]) -> list[str]:
    ...
    for symbol in [
        ...
        if symbol not in lib_text:
            issues.append(f"Rust indexer-core lib missing {symbol}")
    for symbol in ["run_indexer_parity_vectors", "serde_json::to_string_pretty", "process::exit"]:
        if symbol not in bin_text:
            issues.append(f"Rust indexer-core binary missing {symbol}")
    if "crates/indexer-core" not in workspace_text:
        issues.append("core-rs workspace does not include crates/indexer-core")
    if vectors.get("indexer", {}).get("vector_set") != "indexer-core-snapshot-vectors-v1":
        issues.append("indexer vector_set is not indexer-core-snapshot-vectors-v1")
    return issues


def _python_reference(vectors: dict[str, Any]) -> dict[str, dict[str, Any]]:
    results = [item.to_dict() for item in run_indexer_vectors(vectors)]
...
def _run_cargo(timeout: int) -> tuple[int, str, str]:
    cmd = [
        ...
        "-q",
        "-p",
        "netcoin-indexer-core",
        "--bin",
        "netcoin-indexer-parity",
        "--",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run NetCoin Rust indexer-core parity executable and compare with Python"
    )
    parser.add_argument("--out", default="reports/rust_indexer_parity_report.json")
    ...
    report: dict[str, Any] = {
        ...
        "indexer_cases": len(vectors.get("indexer", {}).get("cases", [])),
    ...
    summary_keys = ["ok", "mode", "cargo_available", "schema_version", "indexer_cases", "vector_fingerprint"]
