#!/usr/bin/env python3
"""Run/compare the Rust mempool parity executable against Python reference vectors.
...
and compares each Rust mempool result with the Python reference parity suite.
...
from netcoin.parity_suite import run_mempool_vectors, vector_fingerprint  # noqa: E402

VECTOR_PATH = ROOT / "architecture" / "parity-vectors.json"
RUST_FIXTURE_PATH = ROOT / "core-rs" / "fixtures" / "parity-vectors.json"
RUST_CRATE_PATH = ROOT / "core-rs" / "crates" / "mempool-core"
RUST_BIN_PATH = RUST_CRATE_PATH / "src" / "bin" / "netcoin-mempool-parity.rs"
...
def _source_checks(vectors: dict[str, Any]) -> list[str]:
    ...
    for symbol in [
        ...
        if symbol not in lib_text:
            issues.append(f"Rust mempool lib missing {symbol}")
    for symbol in ["run_mempool_parity_vectors", "serde_json::to_string_pretty", "process::exit"]:
        if symbol not in bin_text:
            issues.append(f"Rust mempool parity binary missing {symbol}")
    if "crates/mempool-core" not in workspace_text:
        issues.append("core-rs workspace does not include crates/mempool-core")
    kinds = {str(case.get("kind")) for case in vectors.get("mempool", {}).get("cases", [])}
    for kind in sorted(kinds):
        if f'"{kind}"' not in lib_text:
            issues.append(f"Rust mempool executable path does not mention vector kind {kind}")
    return issues


def _python_reference(vectors: dict[str, Any]) -> dict[str, dict[str, Any]]:
    results = [item.to_dict() for item in run_mempool_vectors(vectors)]
...
def _run_cargo(timeout: int) -> tuple[int, str, str]:
    cmd = [
        ...
        "-q",
        "-p",
        "netcoin-mempool-core",
        "--bin",
        "netcoin-mempool-parity",
        "--",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run NetCoin Rust mempool parity executable and compare with Python")
    parser.add_argument("--out", default="reports/rust_mempool_parity_report.json")
    ...
    report: dict[str, Any] = {
        ...
        "mempool_cases": len(vectors.get("mempool", {}).get("cases", [])),
    ...
    summary_keys = ["ok", "mode", "cargo_available", "schema_version", "mempool_cases", "vector_fingerprint"]
