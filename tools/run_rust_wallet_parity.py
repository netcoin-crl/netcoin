#!/usr/bin/env python3
"""Run/compare the Rust wallet-core parity executable against Python reference vectors.

When Cargo is available this tool executes:

    cargo run -q -p netcoin-wallet-core --bin netcoin-wallet-parity -- ../architecture/parity-vectors.json

and compares each Rust wallet result with the Python reference parity suite.
Use --allow-missing-cargo or --source-only only for constrained sandboxes that
cannot install Rust; local release gates should run without those flags.
"""
...
from netcoin.parity_suite import run_wallet_vectors, vector_fingerprint  # noqa: E402

VECTOR_PATH = ROOT / "architecture" / "parity-vectors.json"
RUST_FIXTURE_PATH = ROOT / "core-rs" / "fixtures" / "parity-vectors.json"
RUST_CRATE_PATH = ROOT / "core-rs" / "crates" / "wallet-core"
RUST_BIN_PATH = RUST_CRATE_PATH / "src" / "bin" / "netcoin-wallet-parity.rs"
...
def _source_checks(vectors: dict[str, Any]) -> list[str]:
    issues: list[str] = []
    for path in [VECTOR_PATH, RUST_FIXTURE_PATH, RUST_CRATE_PATH / "Cargo.toml", RUST_BIN_PATH, RUST_LIB_PATH]:
        if not path.exists():
    ...
    for symbol in [
        ...
        if symbol not in lib_text:
            issues.append(f"Rust wallet lib missing {symbol}")
    for symbol in ["run_wallet_parity_vectors", "serde_json::to_string_pretty", "process::exit"]:
        if symbol not in bin_text:
            issues.append(f"Rust wallet parity binary missing {symbol}")
    if "crates/wallet-core" not in workspace_text:
        issues.append("core-rs workspace does not include crates/wallet-core")
    if vectors.get("wallet", {}).get("vector_set") != "wallet-core-executable-vectors-v3":
        issues.append("wallet vector_set is not wallet-core-executable-vectors-v3")
    for case in vectors.get("wallet", {}).get("cases", []):
        case_id = str(case.get("id", ""))
        if case_id and case_id not in lib_text and case_id in {"preview-fee-rate-threshold-review"}:
            # At least one named fixture should be present in Rust tests to prove fixture-aware behavior.
            issues.append(f"Rust wallet tests do not mention fixture case {case_id}")
    return issues


def _python_reference(vectors: dict[str, Any]) -> dict[str, dict[str, Any]]:
    results = [item.to_dict() for item in run_wallet_vectors(vectors)]
...
def _compare_with_rust(vectors: dict[str, Any], rust_report: dict[str, Any]) -> list[str]:
    ...
    rust_results = rust_report.get("results", [])
    rust_by_id = {str(item.get("case_id")): item for item in rust_results if isinstance(item, dict)}
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run NetCoin Rust wallet-core parity executable and compare with Python"
    )
    parser.add_argument("--out", default="reports/rust_wallet_parity_report.json")
    parser.add_argument("--source-only", action="store_true", help="validate source/fixtures only; do not run Cargo")
    parser.add_argument(
        "--allow-missing-cargo", action="store_true", help="pass source checks when Cargo is unavailable"
    )
    ...
    report: dict[str, Any] = {
        ...
        "wallet_cases": len(vectors.get("wallet", {}).get("cases", [])),
    ...
    if source_issues:
        ...
        report["note"] = (
            "Cargo was not executed; run without --allow-missing-cargo on a Rust-enabled machine for the live executable comparison."
        )
    elif not cargo_path:
        report["ok"] = False
    ...
    summary_keys = ["ok", "mode", "cargo_available", "schema_version", "wallet_cases", "vector_fingerprint"]
