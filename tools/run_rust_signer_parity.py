#!/usr/bin/env python3
"""Run/compare the Rust signer-core parity executable against Python reference vectors."""
...
RUST_CRATE_PATH = ROOT / "core-rs/crates/signer-core"
RUST_BIN_PATH = RUST_CRATE_PATH / "src" / "bin" / "netcoin-signer-parity.rs"
...
def _source_checks(vectors: dict[str, Any]) -> list[str]:
    ...
    for symbol in [
        ...
        if symbol not in lib_text:
            issues.append(f"Rust signer-core lib missing {symbol}")
    for symbol in ["run_signer_parity_vectors", "serde_json::to_string_pretty", "process::exit"]:
        if symbol not in bin_text:
            issues.append(f"Rust signer-core binary missing {symbol}")
    if "crates/signer-core" not in workspace_text:
        issues.append("core-rs workspace does not include crates/signer-core")
    if vectors.get("signer", {}).get("vector_set") != "signer-core-executable-vectors-v1":
        issues.append("signer vector_set is not signer-core-executable-vectors-v1")
    return issues


def _python_reference(vectors: dict[str, Any]) -> dict[str, dict[str, Any]]:
    results = [item.to_dict() for item in run_signer_vectors(vectors)]
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run NetCoin Rust signer-core parity executable and compare with Python"
    )
    parser.add_argument("--out", default="reports/rust_signer_parity_report.json")
    ...
    report: dict[str, Any] = {
        ...
        "signer_cases": len(vectors.get("signer", {}).get("cases", [])),
    ...
    summary_keys = ["ok", "mode", "cargo_available", "schema_version", "signer_cases", "vector_fingerprint"]
