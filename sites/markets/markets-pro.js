...
(() => {
  ...
  async function renderPortfolio(){
    ...
    $('#content').innerHTML='<section class="trade-card"><h2>Portfolio</h2><label>Trader</label><input id="traderInput" value="'+esc(trader)+'"><button id="loadPortfolio" class="secondary">Load</button>'+posSummary+table(positionRows, [['Market',x=>x.market_id],['Outcome',x=>x.label||x.outcome_id],['Shares',x=>x.quantity],['Mark value',x=>x.mark_value],['Unrealized PnL',x=>pnlHtml(x.unrealized_pnl),'',true]])+'</section><section class="trade-card"><h2>Open orders</h2><div id="openOrdersTable">'+table(myOrders, [['Market',x=>x.market_id],['Outcome',x=>x.outcome_id],['Side',x=>x.side],['Price',x=>x.price],['Remaining',x=>x.remaining??x.quantity],['',x=>'<button type="button" data-cancel="'+esc(x.market_id)+':'+esc(x.order_id)+'">Cancel</button>']])+'</div><p id="portfolioMsg" class="muted"></p></section>';
