...
(() => {
  ...
  function table(rows, cols){rows=rows||[]; if(!rows.length) return '<p class="muted">No rows yet.</p>'; return '<table class="mini-table"><thead><tr>'+cols.map(c=>'<th>'+esc(c[0])+'</th>').join('')+'</tr></thead><tbody>'+rows.map(r=>'<tr>'+cols.map(c=>{const raw=typeof c[1]==='function'?c[1](r):r[c[1]]; return '<td'+(c[2]?' class="'+c[2]+'"':'')+'>'+(c[3]?raw:esc(raw))+'</td>';}).join('')+'</tr>').join('')+'</tbody></table>';}
  function cents(v){const n=Number(v); if(Number.isFinite(n)) return Math.round(n*100)+'¢'; return esc(v||'—');}
  function pnlHtml(v){const n=Number(v); if(!Number.isFinite(n)) return esc(v??'—'); const cls=n>0?'pnl-pos':n<0?'pnl-neg':'pnl-flat'; const sign=n>0?'+':''; return '<span class="'+cls+'">'+sign+n.toFixed(2)+' NET</span>';}
  ...
  async function renderPortfolio(){
    ...
    const totalPnl=positionRows.reduce((s,x)=>s+(Number(x.unrealized_pnl)||0),0);
    const posSummary='<div class="metric-row"><span>Positions '+positionRows.length+'</span><span>Total unrealized PnL '+pnlHtml(totalPnl)+'</span></div>';
    const positionsBody = positionRows.length
      ? posSummary+table(positionRows, [['Market',x=>x.market_id],['Outcome',x=>x.label||x.outcome_id],['Shares',x=>x.quantity],['Mark value',x=>x.mark_value],['Unrealized PnL',x=>pnlHtml(x.unrealized_pnl),'',true]])
      : '<div class="empty-portfolio"><p class="muted">No open positions yet.</p><p class="muted">Buy shares in a market to see your positions and unrealized P&amp;L here.</p><a class="explore-btn" href="index.html">Explore markets</a></div>';
    $('#content').innerHTML='<section class="trade-card portfolio-card"><div class="portfolio-head"><h2>My positions</h2><div class="trader-switch"><label>Trader</label><input id="traderInput" value="'+esc(trader)+'"><button id="loadPortfolio" class="secondary">Load</button></div></div>'+positionsBody+'</section><section class="trade-card orders-card"><h2>Open orders</h2><div id="openOrdersTable">'+(myOrders.length ? table(myOrders, [['Market',x=>x.market_id],['Outcome',x=>x.outcome_id],['Side',x=>x.side],['Price',x=>x.price],['Remaining',x=>x.remaining??x.quantity],['',x=>'<button type="button" data-cancel="'+esc(x.market_id)+':'+esc(x.order_id)+'">Cancel</button>']]) : '<p class="muted">No open orders.</p>')+'</div><p id="portfolioMsg" class="muted"></p></section>';
